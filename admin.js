(function($) {
    ...
    function initCopyToClipboard() {
      $(document).on('click', '.cpg-copy-btn', function(e) {
        e.preventDefault();
        var $btn = $(this);
        var code = $btn.data('code') || $btn.closest('.cpg-shortcode-block, .cpg-usage-shortcode').find('.cpg-code, .cpg-shortcode-display').text();
        if (!code) return;
    ...
    function initDismissibleNotice() {
      $(document).on('click', '.wpcpg-notice-dismiss', function(e) {
        ...
        $notice.slideUp(220, function() { $(this).remove(); });
    ...
    function closeHelpModal() {
      ...
      $modal.fadeOut(120, function() {
        ...
        if ($opener && $opener.length) { try { $opener.focus(); } catch (e) {} }
      });
    }
    ...
    function initStylingControls() {
      ...
      $('.cpg-color-picker, .cpg-style-selector select, .cpg-style-selector input[type="checkbox"]').on('change', function() {
        if ($('#default_user_id').val().trim() !== '') requestPreview();
      });
    }
    ...
    function initUserIdValidation() {
      ...
      function updateValidationStatus(isValid, message, type) {
        ...
        $statusDiv.html('<span class="dashicons dashicons-' + (isValid ? 'yes' : 'no') + '"></span> ' + message);
      ...
      $userIdField.on('input', function() {
        ...
        if (validateUserId(userId)) {
          ...
          updateValidationStatus(false, 'User ID must be numeric, greater than 0, and up to 10 digits', 'invalid');
        }
      });
    ...
    function refreshGalleryPreview() {
      ...
      $preview.html('<div class="cpg-loading"><span class="spinner is-active"></span> Updating preview...</div>');
      ...
      $.post(wpcpgAdmin.ajaxurl, {
        ...
        if (response && response.success) {
          ...
          if (typeof response.data === 'string') {
            ...
            $preview.html('<div class="cpg-error">Invalid preview response</div>');
          }
        } else {
          var msg = (response && response.data) ? response.data : 'Failed to update preview';
          $preview.html('<div class="cpg-error">' + msg + '</div>');
        }
      }).fail(function() {
        $preview.html('<div class="cpg-error">Network error updating preview</div>');
      });
    }
